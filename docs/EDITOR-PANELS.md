...
| Panel | Susunan |
|---|---|
| **Material Editor** | kiri: daftar `.simmat`/`.simmatinst` · tengah: node graph (palet lewat klik kanan) · kanan: tiga tab — **Details** (parameter yang diekspos, properti node terpilih), **Preview** (bola/kubus/bidang yang di-shading shader material itu sendiri, sudut cahaya, eksposur, orbit dengan seret), **Compiled Slang** (kode yang dihasilkan, galat bisa diklik untuk menuju node penyebabnya) · toolbar: Save, Fit, Group selection, New Instance |
...
| **Animation Editor** | kiri: daftar `.simskel`/`.simanim` · kanan: dua tab — **Skeleton** (pohon bone, bind pose, nama rig standar untuk retarget) dan **Clip** (pratinjau rangka 2D dengan pilihan sumbu Front/Side/Top, transport + scrubber, dope sheet dengan baris per bone·kanal berwarna menurut kelompoknya, penyunting kurva bertangen untuk track terpilih, properti kunci, daftar event & penanda fase), · toolbar: Save, undo/redo suntingan klip, play/pause/stop, kecepatan, loop dan **Graph** (kanvas node: simpul state, simpul semu "Any State", transisi sebagai kabel dari pin keluaran ke pin masukan; sisi kanan berisi pratinjau rangka yang digerakkan graph beserta nama state aktif, properti lapis, daftar parameter yang bisa diubah langsung saat pratinjau berjalan, properti motion state terpilih, dan properti transisi terpilih lengkap dengan daftar kondisinya) · viewport 3D dan mesh skinned menunggu E8 |
